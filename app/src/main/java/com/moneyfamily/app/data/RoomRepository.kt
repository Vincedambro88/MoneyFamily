...
class RoomRepository(context: Context) {
    ...
    suspend fun seedDefaults() {
        val initial = linkedMapOf(
            ...
            "RISTORANTE" to "PRANZO/CENA", "CINEMA" to "PRANZO/CENA", "MATERNA" to "SCUOLA", "ELEMENTARI" to "SCUOLA",
            "NUOTO" to "SPORT", "GINNASTICA" to "SPORT", "DANZA" to "SPORT", "STIPENDIO" to "NTT DATA", "BUONI PASTO" to "NTT DATA",
            "TREDICESIMA" to "NTT DATA", "INPS" to "INPS", "730" to "AGENZIA ENTRATE"
        )
...
private fun MovementEntity.toModel() = Movement(
    ...
    amount, category, description, date, member, paymentMethod
)

private fun Movement.toEntity() = MovementEntity(
    id, type.name, amount, category, description, date, member, paymentMethod
)
