    ...
    fun write(context: Context, uri: Uri) {
        context.contentResolver.openOutputStream(uri)?.use { out ->
            ZipOutputStream(out).use { zip ->
                ...
                // Importante: Entrata/Uscita NON sono una tipologia. Il segno dell'importo
                // identifica già entrata (+) o uscita (-). La Tipologia deve corrispondere
                // a una tipologia configurata nell'app.
                val rows = listOf(
                    listOf("Data", "Importo", "Tipologia", "Categoria", "Componente", "Descrizione"),
                    listOf("01/01/2026", "-45,50", "SPESA", "ALIMENTARI", "Papà", "Supermercato"),
                    listOf("05/01/2026", "5700,00", "STIPENDIO", "LAVORO", "Papà", "Stipendio mensile")
                )
                put(zip, "xl/worksheets/sheet1.xml", sheet(rows))
                put(zip, "xl/worksheets/sheet2.xml", sheet(listOf(
                    listOf("Campo", "Come compilarlo"),
                    listOf("Data", "Data dell'operazione nel formato GG/MM/AAAA, ad esempio 05/01/2026"),
                    listOf("Importo", "Numero positivo per un'entrata e negativo per un'uscita, ad esempio 125,50 oppure -45,50"),
                    listOf("Tipologia", "Nome della tipologia configurata nell'app, ad esempio SPESA, STIPENDIO, AFFITTO"),
                    listOf("Categoria", "Nome della categoria configurata nell'app e associata alla tipologia"),
                    listOf("Componente", "Nome del membro della famiglia configurato nell'app"),
                    listOf("Descrizione", "Testo libero che descrive l'operazione")
                )))
            }
