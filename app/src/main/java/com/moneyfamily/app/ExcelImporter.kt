...
object ExcelImporter {
 ...
 private fun validateHeaders(h:List<String>){
  val required=mapOf("Data" to arrayOf("data","date"),"Importo" to arrayOf("importo","amount","valore"),"Tipologia" to arrayOf("tipologia","tipo","type"),"Categoria" to arrayOf("categoria","category"),"Componente" to arrayOf("componente","effettuata da","member","famiglia"),"Descrizione" to arrayOf("descrizione","description"))
 ...
 private fun parseCells(cells:List<String>,h:List<String>,row:Int):ImportedRow{
  ...
  val dateRaw=get("data");val amountRaw=get("importo");val type=get("tipologia");val category=get("categoria");val member=get("componente");val description=get("descrizione")
  ...
  val date=normalizeDate(dateRaw);if(date.isBlank()) error("Riga $row: data non valida")
  val amount=amountRaw.replace(".","").replace(",",".").toDoubleOrNull()?:error("Riga $row: importo non valido")
